...
class Lab(AgentCheck):
    ...
    INCREASE_HOURLY_EXPONENTIAL_CYCLE = {
        'lab.increase_hourly_exponential_cycle.gauge': GAUGE,
        'lab.increase_hourly_exponential_cycle.count': COUNT,
        'lab.increase_hourly_exponential_cycle.rate': RATE
    }
    ...
    def check(self, instance):
        ...
        for metric_name, metric_func in list(self.STEADY1.items()):
        ...
        for metric_name, metric_func in list(self.STEADY0.items()):
        ...
        for metric_name, metric_func in list(self.RAND_10.items()):
        ...
        # range will change with different min_collection_interval
        # rate will likely spike if you restart the agent and at the start of the month
        for metric_name, metric_func in list(self.RAND_FRACT.items()):
        ...
        # shapes will vary
        for metric_name, metric_func in list(self.INCREASE_HOURLY_CYCLE.items()):
            number = float(seconds_this_hour)
        ...
        # shapes will vary
        for metric_name, metric_func in list(self.INCREASE_DAILY_CYCLE.items()):
            number = round_value(hour*(1 + float(minute)/60))
        ...
        # exponentially increasing trends over a day
        for metric_name, metric_func in list(self.INCREASE_HOURLY_EXPONENTIAL_CYCLE.items()):
            x = float(seconds_this_hour/600)
            val = 2*math.exp(x)
            metric_func(self, metric_name, val, tags=tags)
        ...
        # shapes will vary
        for metric_name, metric_func in list(self.INCREASE_WEEKLY_CYCLE.items()):
            number = round_value(day * (1 + float(hour)/24))

            if "rate" in metric_name:
                val = seconds_this_week * (1 + (1/seconds_this_week))
                metric_func(self, metric_name, val*val, tags=tags)
            else:
                metric_func(self, metric_name, number*number, tags=tags)

            counter += 1

        # sparse metrics should show every half hour for about 5 min
        for metric_name, metric_func in list(self.SPARSE.items()):
            number = now.minute
            if number % 30 <= 5:
                metric_func(self, metric_name, number, tags=tags)
                counter += 1
        ...
        for metric_name, metric_func in list(self.TRIG.items()):
            val = 100 * math.sin(seconds_this_month / 100) + 100
            metric_func(self, metric_name, val, tags=tags)
            counter += 1

        # occasional spikes
        for metric_name, metric_func in list(self.SPIKES.items()):
            val = random.random() * 2
        ...
        self.service_check(
            sometimes_red_name,
            sometimes_red_status,
            tags=tags,
            message="Sometimes Red"
            )
        self.log.debug('Service Check {} has status {}'.format(
            sometimes_red_name,
            sometimes_red_status
        ))
        ...
        if send_events:
            ...
            event_title = event.get('title', 'Default Title')
            event_text = event.get('text', 'Default Text')
            event_alert_level = event.get('event_alert_level', 'info')
            if event_alert_level not in ('error', 'warning', 'success', 'info'):
                event_alert_level = 'info'
            event_source_type_name = event.get('event_source_type_name', 'lab')
            event_priority = event.get('event_priority', 'normal')
